...
fn bench_ip_estimate(c: &mut Criterion) {
    ...
    for &dim in DIMS {
        ...
        group.bench_with_input(BenchmarkId::new("turbo", dim), &(code, query), |b, (c, q_vec)| {
            b.iter(|| {
                q.inner_product_estimate(black_box(c), black_box(q_vec)).unwrap()
            })
        });
    }
...
fn bench_l2_estimate(c: &mut Criterion) {
    ...
    for &dim in DIMS {
        ...
        group.bench_with_input(BenchmarkId::new("turbo", dim), &(code, query), |b, (c, q_vec)| {
            b.iter(|| q.l2_distance_estimate(black_box(c), black_box(q_vec)).unwrap())
        });
    }
...
fn bench_compact_deserialize(c: &mut Criterion) {
    ...
    for &dim in DIMS {
        ...
        group.bench_with_input(BenchmarkId::new("from_compact_bytes", dim), &bytes, |b, bytes| {
